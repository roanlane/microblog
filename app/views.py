...
def after_login(login_form):
    user = User.query.filter_by(email = login_form.user_login.data).first()
    if user is None:
        nickname = login_form.user_login.data
        password = login_form.user_password.data
        email = login_form.user_email.data
        nickname = User.make_valid_nickname(nickname)
        nickname = User.make_unique_nickname(nickname)
        user = User(nickname = nickname, password = password, email = email, role = ROLE_USER)
        db.session.add(user)
        db.session.commit()
